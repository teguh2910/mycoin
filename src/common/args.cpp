...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\MyCoin
    //   new: C:\Users\Username\AppData\Local\MyCoin
    // macOS: ~/Library/Application Support/MyCoin
    // Unix-like: ~/.mycoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "MyCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "MyCoin";
#else
    fs::path pathRet;
